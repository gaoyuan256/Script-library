...
#WeChat： youcai222
#Date： 2023年3月16日
#FileName： system_initialization-20230316-V7.0.sh
#URL： https://github.com/gaoyuan256/Script-library/tree/main
#Description： 20230316-V7.0
#********************************************************************
...
then
	green '1、SELinux 关闭成功！'
else
	reds  '1、SELinux 关闭失败！'
fi

green '1、查询确认Disabled为正常关闭！'
green "1、SELinux 当前状态为：`getenforce`！"
yellow '1、请在重启后重新执行以下命令查询：
getenforce'
...
then
	...
	systemctl status NetworkManager | grep inactive &> /dev/null && green '2、停止不必要系统服务成功！'

	#修改系统调优策略
	#设置优化概要文件变量为latency-performance（延迟性能：以增加功耗为代价优化确定性性能）
	tuned_profile='latency-performance'
	tuned-adm profile ${tuned_profile} &> /dev/null && green '2、修改系统调优策略为：'${tuned_profile}'！'
	# tuned-adm list
	# tuned-adm active
...
if [[ ${os_version} == 7 ]]
then
	read -p $'\e[1;32m4、请输入您要修改的主机名（推荐使用小写字母不超过8位）：\e[0m\n' hostname_7
	hostnamectl set-hostname ${hostname_7}
	echo ""
	yellows '4、您的网卡设备列表为：'
	yellow ''${device_list}''
	read -p $'\e[1;32m4、请输入您使用的网卡设备（如  eth0  ）：\e[0m\n' device7
	echo ""
	ipaddr_7=`ifconfig ${device7} |awk -F '[ :]+' 'NR==2{print $3}'`
	...
	then
		...
		reds '4、设置解析失败，请手动调整您的主机名解析！'
	fi
elif [[ ${os_version} == 6 ]]
then
	yellow '4、请注意，Rhel及CentOS系统6.*版本，更改主机名后需重启后永久生效！'
	read -p $'\e[1;32m4、请输入您要修改的主机名（推荐使用小写字母不超过8位）：\e[0m\n' hostname_6
...
green "选择您要使用的功能: "
yellow " 1、我已上传ISO镜像到/software/iso/目录下，现在开始配置YUM源； \n 2. 我已手动挂载ISO镜像到/media/cdrom目录下，现在开始配置YUM源； \n 0. 退出！"
read -p $'\e[1;32m输入数字以选择：\e[0m\n' Function
echo ""
...
EOF
	...
	green "5、Yum仓库信息如下："
	green "5、`yum repolist 2>/dev/null | grep "repolist"`"
	yum install lrzsz -y &> /dev/null
else
	reds '5、镜像挂载失败，请检查！'
	reds '5、请上传ISO镜像到/software/iso/目录下！'
fi

if rpm -qa |grep lrzsz &> /dev/null
then
	green '\e[1;32m5、Yum仓库配置成功！'
else
	reds '\e[1;31m5、Yum仓库配置失败，请检查！'
fi
...
for i in binutils compat-libcap1 gcc gcc-c++ \
...
read -p $'\e[1;32m7、请问您需要现在就开始配置时间同步服务么  (Yy/Nn):  \e[0m\n' ntpcron
echo ""
...
case $ntpcron in
[Yy] )
read -p $'\e[1;32m7、请输入您的时间同步服务端IP：\e[0m\n' ntpserver1
...
echo ""
yellow "7、服务器当前时间为：`date`"
#date

crontab -l &> /dev/null | grep -v "0 1 * * *  $crontab_ntp_job1"; echo "0 1 * * *  $crontab_ntp_job1" | crontab -
#crontab -l
green '7、时间同步服务配置成功！'
yellow '7、检查请执行命令：crontab -l'
else
echo ""
reds '7、时间同步服务配置失败！'
yellow '7、检查请执行命令：crontab -l'
...
;;
[Nn] )
yellow '7、欢迎您下次使用本工具配置时间同步服务！'
#break
;;
* )
reds '7、请输入字母 Y/y或N/n ！'
esac
}
...
&& green '12、透明大页关闭成功，请在重启后重新确认！[enabled]' \
|| reds '12、透明大页关闭失败，请检查！'

echo never > /sys/kernel/mm/transparent_hugepage/defrag \
&& cat /sys/kernel/mm/transparent_hugepage/defrag | grep "\[never\]" &> /dev/null \
&& green '12、透明大页关闭成功，请在重启后重新确认！[defrag]' \
|| reds '12、透明大页关闭失败，请检查！'
...
grep "numa=off" /boot/grub/grub.conf &> /dev/null || sed -i 's/quiet/quiet numa=off/' /boot/grub/grub.conf &> /dev/null
...
then
	...
	grep "transparent_hugepage=never numa=off" /etc/default/grub &> /dev/null || sed -i 's/quiet/quiet transparent_hugepage=never numa=off/' /etc/default/grub
	grub2-mkconfig -o /boot/grub2/grub.cfg &> /dev/null
	grub2-mkconfig -o /boot/efi/EFI/redhat/grub.cfg &> /dev/null
fi

yellows '12、请在重启后重新执行以下命令查询：'
yellows '12、cat /sys/kernel/mm/transparent_hugepage/enabled | grep "\[never\]"'
yellows '12、cat /sys/kernel/mm/transparent_hugepage/defrag | grep "\[never\]"'
yellows '12、grep Huge /proc/meminfo'
yellows '12、请确认AnonHugePages为0正常！'
yellows '12、dmesg | grep -i  "numa turned off"'
yellows '12、cat /proc/cmdline | grep -i "numa=off"'
echo ""
yellows '12、操作系统6版本，额外执行以下命令：'
yellows '12、numactl --hardware | grep -i nodes'
yellows '12、请确认available为2或多个nodes就说明numa没关掉！'
echo ""
...
read -p $'\e[1;32m13、请输入您想选择的存储设备（如sda）： \e[0m\n' device_scheduler
...
read -p $'\e[1;32m13、请输入您想选择的调度算法（如noop）： \e[0m\n' scheduler_name

echo $scheduler_name > /sys/block/$device_scheduler/queue/scheduler \
&& green '13、'$scheduler_name'调度算法设置成功！' \
|| reds '13、'$scheduler_name'调度算法设置失败！'
...
yellow '13、请在重启后重新执行以下命令查询：'
yellow '13、cat /sys/block/'$device_scheduler'/queue/scheduler |grep '\[$scheduler_name\]''
...
echo -e "\033[44;37m$RESULTFILE \033[0m"
;;
'"18"')
close_firewalld_selinux
stop_os_service
