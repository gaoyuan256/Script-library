...
#WeChat： youcai222
#Date： 2021年10月25日
#FileName： system_initialization-20211025-V5.6.sh
#URL： http://saonian.tpddns.cn:8686/nextcloud/index.php/apps/dashboard/
#Description： 2021年12月2日 -V6.0
#********************************************************************
...
$$$  / \$$$ |$$   ____|$$ |       $$  /    $$ |  $$ |$$ |
$$  /   \$$ |\$$$$$$$\ $$ |      $$$$$$$$\ $$ |  $$ |$$ |
\__/     \__| \_______|\__|      \________|\__|  \__|\__|



                     Auther: GaoYuan                       
            Creation time: 2021/12/02    -V6.0        '
echo ""
echo ""
echo ""
...
if [[ `id | sed -r -n "s/uid=([0-9]{1,4}).*/\1/p"` != 0 ]];then
  echo -e '\e[1;31m当前不是Root用户，不允许设置此优化！\e[0m\n'
  exit
fi
...
if [ ! -n "$os_version" ]; then
  echo -e '\e[1;31m版本检索失败，请手工确认！\e[0m\n'
  exit
fi


#确认Linux系统版本为6或者7，从而使用不同的系统命令
if [ -f "/etc/centos-release" ]
  then
    echo -e '\e[1;35m当前操作系统版本为： CentOS-'$os_version'\e[0m\n'
  else
    if [ -f "/etc/redhat-release" ]
      then
      echo -e '\e[1;35m当前操作系统版本为： RHEL-'$os_version'\e[0m\n'
    fi
fi
...
yum_config() {
  ...
  if [ "${Function}" == "1" ] || [ "${Function}" == "2" ]; then
    ...
    then 
      ...
      cat <<END > /etc/yum.repos.d/weizhi_dvd.repo
[weizhi_dvd]
name=weizhi_dvd
...
ssh_adjust(){
  ...
  if grep "UseDNS no" /etc/ssh/sshd_config &> /dev/null; then
    echo -e '\e[1;32m8、SSH加速调整成功！\e[0m\n'
  else
    echo -e '\e[1;31m8、SSH加速调整失败，请检查！\e[0m\n'
  fi
...
EOF
  ...
  && echo -e '\e[1;32m9、系统资源限制修改成功！\e[0m\n' \
  || echo -e '\e[1;31m9、系统资源限制修改失败！\e[0m\n'
...
EOF
  ...
  && echo -e '\e[1;32m10、系统内核参数配置成功！\e[0m\n' \
  || echo -e '\e[1;31m10、系统内核参数配置失败！\e[0m\n'
...
EOF
  ...
  && echo -e '\e[1;32m11、系统全局环境变量配置成功！\e[0m\n' \
  || echo -e '\e[1;31m11、系统全局环境变量配置失败！\e[0m\n'
}
...
EOF
  ...
  echo -e '\e[1;32m12、请在重启后重新执行以下命令查询：\e[0m\n'
  echo -e '\e[1;33m12、cat /sys/kernel/mm/transparent_hugepage/enabled | grep "\[never\]"\e[0m\n'
  echo -e '\e[1;33m12、cat /sys/kernel/mm/transparent_hugepage/defrag | grep "\[never\]"\e[0m\n'
  echo -e '\e[1;33m12、grep Huge /proc/meminfo\e[0m\n'
  echo -e '\e[1;32m12、请确认\e[1;33mAnonHugePages\e[0m\e[1;32m为\e[0m\e[1;33m0\e[0m\e[1;32m正常！\e[0m\n'
  echo -e '\e[1;33m12、dmesg | grep -i  "numa turned off"\e[0m\n'
  echo -e '\e[1;33m12、cat /proc/cmdline | grep -i "numa=off"\e[0m\n'
  echo -e '\e[1;33m12、numactl --hardware | grep -i nodes\e[0m\n'
  echo -e '\e[1;32m12、请确认\e[1;33mavailable: 1 nodes (0)\e[0m\e[1;32m为\e[0m\e[1;33m2或多个nodes就说明numa\e[0m\e[1;32m没关掉！\e[0m\n'
}
...
if (whiptail --title "上海维致信息技术有限公司-高原" --yes-button "开始" --no-button "退出"  --yesno "       欢迎您使用操作系统初始化程序，请问要现在开始么？" 10 65) then
    ...
    echo -e '\e[1;34m您选择了退出,期待您的下次使用！\e[0m\n'
    echo ""
    echo ""
    exit
fi
